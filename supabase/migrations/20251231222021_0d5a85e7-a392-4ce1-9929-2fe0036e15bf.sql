-- Update the blog post translation cache warming function to also warm meta_description
...
BEGIN
  ...
  IF (TG_OP = 'INSERT' AND NEW.status = 'published') OR 
     (TG_OP = 'UPDATE' AND NEW.status = 'published' AND 
      (OLD.status != 'published' OR OLD.content != NEW.content OR OLD.title != NEW.title OR OLD.meta_description IS DISTINCT FROM NEW.meta_description)) THEN
    ...
    -- Queue HTTP request to warm cache for meta_description (if present)
    IF NEW.meta_description IS NOT NULL AND NEW.meta_description != '' THEN
      SELECT extensions.net.http_post(
        ...
        body := jsonb_build_object(
          'pageId', NEW.id || '-meta',
          'content', NEW.meta_description,
          'languages', ARRAY['fr', 'es', 'pt', 'de', 'it', 'zh', 'ar', 'nl']
        )
      ) INTO request_id;
      
      RAISE LOG 'Queued translation cache warming for blog post meta_description: % (request_id: %)', NEW.slug, request_id;
    END IF;
